...
export type SolutionFilter = (solution: Solution) => boolean;
...
export type SatisfactionsMakerOptions = {
  isSolutionAllowed?: SolutionFilter | undefined;
  isSignatureKnown?: ((key: string) => boolean) | undefined;
  maxSolutions?: number | null | undefined;
};

type CombineOptions = {
  isSolutionAllowed?: SolutionFilter | undefined;
  solutionCounter?: SolutionCounter | undefined;
};

function incrementSolutionCounter(counter?: SolutionCounter): void {
  if (!counter) return;
  if (counter.count + 1 > counter.maxSolutions) {
    throw new Error(`Satisfactions limit exceeded (${counter.maxSolutions}).`);
  }
  counter.count += 1;
}

/**
 * Pushes a solution into solutions after applying the optional
 * allow/limit checks.
 * This function will throw if the limit is hit
 */
function pushAllowedSolution(
  solutions: Solution[],
  solution: Solution,
  isSolutionAllowed?: SolutionFilter,
  solutionCounter?: SolutionCounter
): void {
  if (isSolutionAllowed && !isSolutionAllowed(solution)) {
    return;
  }
...
/**
 * Enumerates every key combination of size `k` from the provided `keys` array.
 *
 * `multi` and `multi_a` use this to enumerate signature subsets while
 * constructing satisfactions. The `onKeyCombination` callback is invoked with
 * each combination in the order they are discovered.
 */
function forEachKeyCombination(
  /** The list of candidate keys to choose from. */
  keys: string[],
  /** The number of elements to select per combination. */
  k: number,
  /** The starting index for enumeration. */
  start: number,
  /** The current combination being built. */
  currentKeyCombination: string[],
  /** Callback invoked with each completed combination. */
...
function combineSolutions(
  ...
  { isSolutionAllowed, solutionCounter }: CombineOptions = {}
): Solution[] {
  ...
  //
  //Recursively call combineSolutions(post) until there are no further
  //sat()/dsat() in *post*
  ...
  if (currMatch && currMatch.length) {
    ...
    for (const currSolution of currSolutions) {
      ...
      if (post.match(reCurr)) {
        ...
        //This was the last instance of combineSolutions where there are no *post*
        //sat/dsats
        const solution: Solution = {
          ...currSolution,
...
/**
 * Creates a satisfactions maker with optional pruning and enumeration limits.
 *
 * The returned object mirrors the Miniscript fragment set and provides helper
 * functions that return sat/dsat solutions for each fragment. When
 * `isSolutionAllowed` is provided, solutions that do not pass the filter are
 * skipped during enumeration. When `maxSolutions` is set, enumeration throws
 * once the counter exceeds that limit. `isSignatureKnown` lets multisig
 * fragments (`multi`, `multi_a`) ignore keys that are known to be unavailable.
 */
export const createSatisfactionsMaker = ({
  isSolutionAllowed,
  isSignatureKnown,
  maxSolutions
}: SatisfactionsMakerOptions = {}) => {
  const solutionCounter =
    typeof maxSolutions === 'number' ? { maxSolutions, count: 0 } : undefined;
  const combineFilteredSolutions = (
    solutionTemplate: string,
    satisfactionsMap: SatisfactionsMap
  ) =>
    combineSolutions(solutionTemplate, satisfactionsMap, {
      isSolutionAllowed,
      solutionCounter
    });
  const filterSolutions = (solutions: Solution[]): Solution[] => {
    if (!isSolutionAllowed && !solutionCounter) return solutions;
    const filteredSolutions: Solution[] = [];
    for (const solution of solutions) {
      const isAllowed = isSolutionAllowed ? isSolutionAllowed(solution) : true;
      if (isAllowed) {
        incrementSolutionCounter(solutionCounter);
  ...
  /**
   * An object containing functions for generating Basic satisfaction and
   * dissatisfaction sets for miniscripts.
   *
  ...
  const satisfactionsMaker = {
    0: (): Satisfactions => ({
      dsats: filterSolutions([{ asm: `` }])
    }),
    1: (): Satisfactions => ({
      sats: filterSolutions([{ asm: `` }])
    }),
    pk_k: (key: string): Satisfactions => ({
      dsats: filterSolutions([{ asm: `0` }]),
      sats: filterSolutions([{ asm: `<sig(${key})>` }])
    }),
    pk_h: (key: string): Satisfactions => ({
      dsats: filterSolutions([{ asm: `0 <${key}>` }]),
    ...
    ripemd160: (h: string): Satisfactions => ({
      sats: filterSolutions([{ asm: `<ripemd160_preimage(${h})>` }]),
      dsats: filterSolutions([{ asm: `<random_preimage()>` }])
    }),
    hash256: (h: string): Satisfactions => ({
      sats: filterSolutions([{ asm: `<hash256_preimage(${h})>` }]),
      dsats: filterSolutions([{ asm: `<random_preimage()>` }])
    }),
    ...
    andor: (
      ...
      dsats: [
        ...combineFilteredSolutions(`dsat(Z) dsat(X)`, { X, Y, Z }),
        ...combineFilteredSolutions(`dsat(Y) sat(X)`, { X, Y, Z })
      ],
      sats: [
        ...combineFilteredSolutions('sat(Y) sat(X)', { X, Y, Z }),
        ...combineFilteredSolutions('sat(Z) dsat(X)', { X, Y, Z })
      ]
    }),
    and_v: (X: Satisfactions, Y: Satisfactions): Satisfactions => ({
      dsats: [...combineFilteredSolutions(`dsat(Y) sat(X)`, { X, Y })],
      sats: [...combineFilteredSolutions(`sat(Y) sat(X)`, { X, Y })]
    }),
    and_b: (X: Satisfactions, Y: Satisfactions): Satisfactions => ({
      dsats: [
        ...combineFilteredSolutions(`dsat(Y) dsat(X)`, { X, Y }),
        ...combineFilteredSolutions(`sat(Y) dsat(X)`, { X, Y }),
        ...combineFilteredSolutions(`dsat(Y) sat(X)`, { X, Y })
      ],
      sats: [...combineFilteredSolutions(`sat(Y) sat(X)`, { Y, X })]
    }),
    or_b: (X: Satisfactions, Z: Satisfactions): Satisfactions => ({
      dsats: [...combineFilteredSolutions(`dsat(Z) dsat(X)`, { X, Z })],
      sats: [
        ...combineFilteredSolutions(`dsat(Z) sat(X)`, { X, Z }),
        ...combineFilteredSolutions(`sat(Z) dsat(X)`, { X, Z }),
        ...combineFilteredSolutions(`sat(Z) sat(X)`, { X, Z })
      ]
    }),
    or_c: (X: Satisfactions, Z: Satisfactions): Satisfactions => ({
      sats: [
        ...combineFilteredSolutions(`sat(X)`, { X, Z }),
        ...combineFilteredSolutions(`sat(Z) dsat(X)`, { X, Z })
      ]
    }),
    ...
    or_i: (X: Satisfactions, Z: Satisfactions): Satisfactions => ({
      dsats: [
        ...combineFilteredSolutions(`dsat(X) 1`, { X, Z }),
        ...combineFilteredSolutions(`dsat(Z) 0`, { X, Z })
      ],
      sats: [
        ...combineFilteredSolutions(`sat(X) 1`, { X, Z }),
        ...combineFilteredSolutions(`sat(Z) 0`, { X, Z })
      ]
    }),
    /*
    ...
    thresh: (
      ...
      for (const solutionTemplate of dsatsNonCanSolutionTemplates) {
        dsats.push(
          ...combineFilteredSolutions(solutionTemplate, satisfactionsMap)
        );
      }
    ...
    multi: (k: string | number, ...keys: string[]): Satisfactions => {
      ...
      forEachKeyCombination(candidateKeys, k, 0, [], keyCombination => {
        ...
        pushAllowedSolution(sats, { asm }, isSolutionAllowed, solutionCounter);
      });

      return { sats, dsats };
    },
    multi_a: (k: string | number, ...keys: string[]): Satisfactions => {
      //multi_a must map all keys and conditionally insert sig/0,
      if (Number.isInteger(Number(k)) && Number(k) > 0) k = Number(k);
      else throw new Error(`k must be positive number: ${k}`);
    ...
    a: (X: Satisfactions): Satisfactions => ({
      dsats: [...combineFilteredSolutions(`dsat(X)`, { X })],
      sats: [...combineFilteredSolutions(`sat(X)`, { X })]
    }),
    ...
    d: (X: Satisfactions): Satisfactions => ({
      dsats: filterSolutions([{ asm: `0` }]),
      sats: [...combineFilteredSolutions(`sat(X) 1`, { X })]
    }),
   ...
   */
    j: (X: Satisfactions): Satisfactions => {
      ...
      return { dsats, sats: [...combineFilteredSolutions(`sat(X)`, { X })] };
    },
...
export type SatisfactionsMaker = ReturnType<typeof createSatisfactionsMaker>;
